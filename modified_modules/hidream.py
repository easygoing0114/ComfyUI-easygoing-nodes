...
class HiDreamTEModel(torch.nn.Module):
    def __init__(self, clip_l=True, clip_g=True, t5=True, llama=True, dtype_t5=None, dtype_llama=None, dtype=None, device="cpu", model_options={}):
        super().__init__()
        self.dtypes = set()
        device = "cpu"  # Force all processing to CPU
        if clip_l:
        ...
        logging.debug("Created HiDream text encoder with: clip_l {}, clip_g {}, t5xxl {}:{}, llama {}:{} on device {}".format(clip_l, clip_g, t5, dtype_t5, llama, dtype_llama, device))
    ...
    def encode_token_weights(self, token_weight_pairs):
        ...
        if len(token_weight_pairs_g) > 0 or len(token_weight_pairs_l) > 0:
            if self.clip_l is not None:
                ...
                l_pooled = torch.zeros((1, 768), device="cpu")  # Use CPU device

            if self.clip_g is not None:
                g_out, g_pooled = self.clip_g.encode_token_weights(token_weight_pairs_g)
            else:
                g_pooled = torch.zeros((1, 1280), device="cpu")  # Use CPU device
        ...
        if t5_out is None:
            t5_out = torch.zeros((1, 128, 4096), device="cpu")  # Use CPU device

        if ll_out is None:
            ll_out = torch.zeros((1, 32, 1, 4096), device="cpu")  # Use CPU device

        if pooled is None:
            pooled = torch.zeros((1, 768 + 1280), device="cpu")  # Use CPU device
...
def hidream_clip(clip_l=True, clip_g=True, t5=True, llama=True, dtype_t5=None, dtype_llama=None, t5xxl_scaled_fp8=None, llama_scaled_fp8=None):
    class HiDreamTEModel_(HiDreamTEModel):
        def __init__(self, dtype=None, device="cpu", model_options={}):
